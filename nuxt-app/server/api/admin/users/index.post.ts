...
export default defineEventHandler(async (event) => {
  ...
  const groups: string[] = Array.isArray(body.groups) ? Array.from(new Set<string>(body.groups.map((value: unknown) => String(value)).map((value: string) => value.trim()).filter(Boolean))) : []
