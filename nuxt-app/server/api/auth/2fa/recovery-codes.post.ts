...
export default defineEventHandler(async (event) => {
  ...
  if (!current?.twoFactorEnabled || !current.twoFactorSecret || !verifyTotp(current.twoFactorSecret, code)) throw createError({ statusCode: 400, message: 'Invalid two-factor code' })
  const recovery = generateRecoveryCodes()
  await prisma.user.update({ where: { id: user.id }, data: { twoFactorRecoveryCodes: JSON.stringify(recovery.hashed), tokenVersion: { increment: 1 } } })
