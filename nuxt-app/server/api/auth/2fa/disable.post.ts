...
export default defineEventHandler(async (event) => {
  ...
  await prisma.user.update({ where: { id: user.id }, data: { twoFactorEnabled: false, twoFactorSecret: null, twoFactorPendingSecret: null, twoFactorRecoveryCodes: null, tokenVersion: { increment: 1 } } })
  return { success: true }
})
