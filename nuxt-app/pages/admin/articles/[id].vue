...
onMounted(async () => {
  try {
    const [article, tags] = await Promise.all([
      authFetch<any>(`/api/articles/${articleId}`),
      authFetch<any[]>('/api/tags').catch(() => []),
    ])
    ...
    form.tags = article.tags?.map((t: any) => t.name) || []
    existingTags.value = tags.map((t: any) => ({ id: t.id, name: t.name }))
  } catch (e: any) {
    ElMessage.error(e.data?.message || 'Failed to load article')
...
async function handleSubmit() {
  ...
  try {
    ...
    ElMessage.error(e.data?.message || 'Failed to update article')
  } finally {
    saving.value = false
  }
}
...
async function handleDelete() {
  try {
    await ElMessageBox.confirm(
      'This will permanently delete this article. Continue?',
      'Confirm Delete',
  ...
  deleting.value = true
  try {
    await authFetch(`/api/articles/${articleId}`, { method: 'DELETE' })
    ElMessage.success('Article deleted')
    router.push('/admin/articles')
  } catch (e: any) {
    ElMessage.error(e.data?.message || 'Failed to delete article')
  } finally {
...
<template>
  <div v-loading="loading" class="article-form-page">
